    ...
    public class MobileAppsConfiguration : IExtensionConfigProvider
    {
        ...
        public void Initialize(ExtensionConfigContext context)
        {
            ...
            rule.WhenIsNotNull(nameof(MobileTableAttribute.TableName)).
                BindToInput<IMobileServiceTableQuery<JObject>>(typeof(MobileTableQueryBuilder<>), this);
            rule.BindToInput<IMobileServiceTableQuery<MobileType>>(typeof(MobileTableQueryBuilder<>), this);

            rule.BindToInput<IMobileServiceTable>(new MobileTableJObjectTableBuilder(this));

            rule.WhenIsNotNull(nameof(MobileTableAttribute.TableName)).
                BindToInput<IMobileServiceTable<JObject>>(typeof(MobileTablePocoTableBuilder<>), this);
            rule.BindToInput<IMobileServiceTable<MobileType>>(typeof(MobileTablePocoTableBuilder<>), this);

            rule.WhenIsNotNull(nameof(MobileTableAttribute.TableName)).
                BindToValueProvider<JObject>(BindForItemAsync);
            rule.BindToValueProvider<MobileType>(BindForItemAsync);
        }

        // Must have an single 'string Id' property 
        internal class MobileType : OpenType
        {
            // We alreay had a previous rule check for the JObject case. 
            public override bool IsMatch(Type type, OpenTypeMatchContext context)
            {
                // POCO types must have a string id property (case insensitive).
                IEnumerable<PropertyInfo> idProperties = type.GetProperties()
                    .Where(p => string.Equals("id", p.Name, StringComparison.OrdinalIgnoreCase) && p.PropertyType == typeof(string));

                return idProperties.Count() == 1;
            }
        }
